...
Five of twenty-five stories are built; three of them are awaiting their human checkpoint. What runs today:
...
- **XLSX and Parquet Sources.** The formats that carry real types where CSV carries strings. A column the file already typed arrives pre-typed, skips the locale question, and still faces the confirmation gate — including the sweep that names the stray string in a number column and the order number whose digits no JavaScript number can hold. Multi-sheet workbooks propose a sheet and list the rest; a switch re-reads the file, keeping your notes and dropping the confirmation. Parquet has no parse controls at all, because its schema is the answer. Legacy `.xls`, `.xlsb` and `.ods` are refused by name rather than half-read.
...
- **Typing as Step zero.** A type and, where relevant, a reading proposed per column by reading *every* value — not a sample, which is how comparable engines corrupt large files silently. Where the evidence decides, the deciding count is named — and the count pointing the other way, if there is one; where nothing in a column settles it, the interface says exactly that instead of picking, and the Source cannot be confirmed until a person answers. A column whose two readings would give the same number, such as one of plain integers, is not a question and is never asked as one. Free-text annotations per column.
- **The types a report actually holds.** Beyond text, numbers and dates: timestamps down to the nanosecond, in the shapes SQL Server, Postgres, Excel and ISO 8601 really write; clock times and durations, which are asked apart rather than guessed, because nothing under 24:00 tells them apart; `ja`/`nein` and `WAHR`/`FALSCH` flags; amounts carrying `%`, `€` or `$`, where the unit rides on the column and the number stays the number in the field; and the accounting negatives `(1.234,56)` and `1.234,56-`, read as negative amounts where the column carries a decimal or grouping mark to vouch for them — so an ERP part number `4711-` and a footnote marker `(1)` stay text rather than becoming confident negatives. Two-digit years read against Excel's fixed century pivot, on the German `TT.MM.JJ` shape — and a column of `01.02.03` is asked about rather than dated, because a version number has the same shape. A nineteen-digit order number stays text: its digits are the information, and no JavaScript number can hold them.
