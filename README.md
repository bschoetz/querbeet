...
- **The types a report actually holds.** Beyond text, numbers and dates: timestamps down to the nanosecond, in the shapes SQL Server, Postgres, Excel and ISO 8601 really write; clock times and durations, which are asked apart rather than guessed, because nothing under 24:00 tells them apart; `ja`/`nein` and `WAHR`/`FALSCH` flags; amounts carrying `%`, `€` or `$`, where the unit rides on the column and the number stays the number in the field; and the accounting negatives `(1.234,56)` and `1.234,56-`, whose sign is carried rather than merely stripped. Two-digit years read against Excel's fixed century pivot — and a column of `01.02.03` is asked about rather than dated, because a version number has the same shape. A nineteen-digit order number stays text: its digits are the information, and no JavaScript number can hold them.
