...
4. ~~**Should a row order ever be data rather than view?**~~ **Answered 2026-08-04 by the project owner: yes.** Two Steps — *Sortieren* and *Erste N* — because the case that decided it (take the 10 newest and carry them on) needs both. No promotion from the view into a Step: viewing and transformation stay separate. Recorded with its open points in `stub-sort-and-limit-steps.md`, which is now closed: story 6d was cut from it and shipped the same day, with CAP-40 in the capability set and a flag for taking the rows from either end of the order.
