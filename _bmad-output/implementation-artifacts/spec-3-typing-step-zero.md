...
- **The equivalence rule is a separator test, not a conversion.** Two number readings differ only where a value carries one of their separators. If no value in the column contains `.` or `,`, every value is a signed digit string that both readings parse to the same number — so the test is exact and costs one pass, and nothing is converted, which keeps the story's "Never" intact. The same equivalence exists in principle for dates whose day and month coincide in every value (`01.01.2025`, `02.02.2025`), and it is deliberately not implemented: deciding it needs the two readings compared as values, that means parsing, and parsing is story 6. Such a column stays `unresolved` and the user answers a question that at least has an answer.
