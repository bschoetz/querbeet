...
- **The box representation is private to `adapters/arquero/`** (AD-22), and **core never constructs one**: conversion hands the adapter each column as `{name, type, values, unparsed}` — at the `unparsed` indices `values` holds the original text, and the adapter boxes exactly those positions. **At the edges a boxed cell materializes as its original text** — the same text export writes (AD-22); box identity is observable only through engine operations and the `unparsed` map, never through `rows()`/`column()`. The `Table` interface stays exactly `rows() / rowCount() / schema() / column(name)` (AD-5); `schema()` reports the confirmed types. What the UI needs for marking crosses as plain data: conversion returns `{ table, unparsed }` where `unparsed` maps column name → frozen array of row indices.
...
- 2026-08-04 (pre-dev seam review, owner-approved): `fromColumns` gained the `unparsed` channel — core cannot construct a box (adapter-private, AD-22), so the port must carry which positions the adapter boxes; the box's edge representation is now stated (original text at `rows()`/`column()`, identity only via engine ops and the `unparsed` map). KEEP: the Step-zero cache decision is unchanged.
