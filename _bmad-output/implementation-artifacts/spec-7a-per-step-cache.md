...
**Review round 2 — the round-1 fixes, finished (see Spec Change Log). Sharpen; do not replace.**

- [x] `ui/App.test.js` -- make the removal case observe the cache instead of the engine: mount `App` with a real `createRunCache()` the test holds, run the graph, remove the Source on the Sources tab, assert `cache.size() === 0`; add the same for an unconfirm. The current assertion (`engine.calls.filter === afterFirstRun`) holds whether or not `clear()` ran, because a removed Source refuses at the frontier -- **deleting `:run-cache="runCache"` from `ui/App.vue` must turn a test red**, which today it does not
- [x] `ui/SourcesPane.vue` + `ui/SourcesPane.test.js` -- clear the run cache from **every** command that commits `confirmed: false`, not two of five: `setColumnTyping`'s callers (`setType`, `setFormat`, `setMissing`) and both re-parses (`reconfigureParse`, `overrideEncoding`) alongside the existing `remove` and `unconfirm`; rename the "leaves it alone for every command that withdraws nothing" case to what it covers, and cover the re-parse and set-type paths -- AD-29 is about the withdrawal, and a re-parse withdraws
- [x] `core/exec/execute.js` + test -- wrap the `sourceKey(node.id)` call at `:260` in `keyOrNull` as its neighbour already is; store on `results.get(node.id).table !== null` rather than on `outcome.table !== null`, so the condition reads the value actually stored -- a door that throws is not a door
- [x] `core/exec/cache-key.js` + `cache-key.test.js` -- emit the refusal **once per distinct message** (the architecture's amended logging row now requires it: `stepZeroKey` runs per Source per run, so an un-deduplicated warning fires at keystroke frequency); narrow the `catch` so `stepKey`'s and `sourceKey`'s documented caller-bug guards are not swallowed with `canonical`'s refusals, or state in the doc why one outcome for three classes is right; give `keyOrNull` its own cases including the non-`Error` branch, which today interpolates `[object Object]` -- it is exported and carries the round-1 fix
- [x] `core/exec/cache.js` + `cache.test.js` -- replace `DEFAULT_MAX_ENTRIES`'s false rationale with the arithmetic: at 1,000 entries the ceiling binds first for anything under 15,000 rows per entry, which is the ordinary case, so say what the number is *for* (a slot bound that a zero-row entry cannot evade) and pick it deliberately or raise it; the defending test currently re-asserts the constant rather than a property -- a comment that the code disproves is worse than no comment
- [x] `core/exec/execute.test.js` -- assert the survivor the eviction case's corrected comment names (`e.calls.selectColumns`), not only that one entry survived -- the round-1 correction replaced a wrong comment with an unchecked one
- [x] `_bmad-output/implementation-artifacts/deferred-work.md` -- append two entries: a zero-row Filter result costs zero against both ceilings while pinning its parent's columns against GC, so neither bound is a bound on bytes; and the bundled, minified `digestBytes` is never checked against the Node implementation's output, which `npm run test:e2e` cannot do

**Review round 3 — the recurring hole closed at its root, and two instructions of mine reconciled (see Spec Change Log). No mechanism changes.**

- [x] `ui/App.test.js` -- at least one case must mount `App` **the way `app/main.js` mounts it**: no `runCache` prop, so App's own default factory is what is under test, and observe caching through the engine across a view switch. The three cases that need a handle to read `size()` from keep injecting -- verified twice here: `default: null` and `default: () => createRunCache({ maxEntries: 1 })` both leave all 1027 tests green, so the shipped artefact's only cache is the one expression nothing covers
- [x] `core/exec/execute.js` + `core/exec/cache-key.js` + tests -- separate the two trust boundaries round 2 conflated. **A door is foreign code on a render path: contain everything it throws.** The `sourceKey` callback gets that treatment; internal `stepKey` calls keep `keyOrNull`'s narrow `CanonicalRefusal`-only containment so a caller bug still propagates. Test the door with something that is *not* a `CanonicalRefusal` — the current case picks `canonical({ at: new Date(0) })`, the one class the containment already covers, so it certifies the fix without exercising its stated purpose -- probed here: an injected `sourceKey` throwing either its own documented guard or a plain `Error` escapes `executeGraph` today
- [x] `ui/SourcesPane.vue` + `ui/SourcesPane.test.js` -- clear only when the re-parse actually committed something (`const committed = await run(); … if (committed !== undefined) withdraw()`), so the three no-op paths stop discarding the cache: `setDelimiter('')`, `setSheet('')` and `setHeaderRow` with any non-integer or `< 1`. Keep the `finally` placement for the failed-read case, which is the reason it is there. Cover `setHeaderRow` and `setSheet` in the withdrawal block — only delimiter and encoding have cases — and fix the comment above `reparse` that still says "the three commands that re-parse" while four handlers call it -- a keystroke in the Kopfzeile field currently throws away the whole cache
- [x] `eslint.config.js` -- add `no-console` to the `core/**/*.js` block with an override permitting it in `core/exec/cache-key.js`, so the register the architecture's logging row points at is actually a gate. Today `globalThis.console.warn` in any core file passes lint clean, which makes both that row and this file's own comment assert an enforcement that does not exist -- the rule must become true or the claim must come out, and it is the claim that is wrong
- [x] `core/exec/cache-key.js` + `cache-key.test.js` -- identify a refusal by a tag (`refusal?.name === 'CanonicalRefusal'` or an own field) rather than by `instanceof`, which is per module instance: two copies of this module in one graph would rethrow every refusal onto a render path, a strictly worse failure than the blanket catch it replaced. Cap `reported` and correct its docstring — it claims to be bounded by distinct paths, but `canonical` builds array paths as `${path}[${i}]`, so a wide typing or a foreign Recipe mints one message per index. Move `forgetRefusals` to a `beforeEach` in each of the three suites that need it, rather than five hand calls whose omission makes a case pass on its position in the file
- [x] `core/exec/cache.test.js` -- pin `DEFAULT_MAX_ENTRIES` to its value beside the crossover property. The replacement computes the crossover from the constants themselves, so setting it to 15,000 — the exact change the code comment says was deliberately not made — still passes -- round 2's charge against round 1 was that the test re-asserted the constant; re-asserting the ratio has the same hole
- [x] `ui/App.test.js` -- assert the typing was actually withdrawn in the unconfirm case (`store.get(sourceId).typing.confirmed === false`), not only that the cache emptied; restore the derived remove-selector so a fixture rename fails as a name mismatch rather than as an empty find -- its sibling already checks the store, and the pair should be symmetric

**Acceptance Criteria:**

- Given `App` mounted with no `runCache` prop, exactly as `app/main.js` mounts it, when a graph runs and the Editor is unmounted and remounted by a view switch, then no Step recomputes.
- Given an injected `sourceKey` that throws anything at all, when the graph runs, then nothing escapes `executeGraph`.
- Given a re-parse control given a value that issues no store command, when it is used, then the run cache still holds what it held.
- Given `ui/App.vue` with either cache binding removed, when the suite runs, then at least one test fails.
- Given a confirmed Source whose typing is withdrawn by any command that commits `confirmed: false`, when that command completes, then the run cache holds nothing.
...
**2026-08-05 — review round 2. The round-1 fixes were right and three of them were half-applied; four of the misses were this spec's wording rather than the code.**

*Triggering findings.* The three layers ran again over `git diff e703340 HEAD`. The four edge classes are genuinely closed — the crash, the entry ceiling, the release path and the null-table store all have tests that fail on the obvious mutation. What round 2 found is that two of the fixes stop short of their own rule, one repeats the verification hole that motivated it, and this spec acquired four self-contradictions when it was amended in round 1.

1. **Verified by mutation here, not taken on report:** deleting `:run-cache="runCache"` from `ui/App.vue` leaves all 1015 tests green. Round 0's finding was this exact shape for `:cache`; `ui/App.test.js` was written to close it and closed it only for the prop that existed when it was written. Its removal case asserts `engine.calls.filter === afterFirstRun`, which holds whether the cache was cleared, not cleared, or never passed — after a `remove` the run refuses at the frontier and calls the engine zero times either way.
2. **`clear()` covers two of four withdrawal paths.** *(Corrected in round 3: this entry originally said five and cited `:844`. `core/exec/source-store.js:844` is `annotateColumn`, which passes `entry.typing.confirmed` through and is documented as "the one column edit that leaves a confirmation standing" — it is not a withdrawal, and the round-2 test asserting that an annotation clears nothing is right. The code was implemented against the rule and is correct; this evidence line was not.)* The withdrawals are `core/exec/source-store.js:440` (`retype`, reached from **every** `reRead`, so `reconfigureParse` and `overrideEncoding`), `:824` (`withColumn(..., false)` from `setColumnTyping`), `:874` (the explicit `unconfirm`), and `removeSource`. `props.runCache?.clear()` sits at `ui/SourcesPane.vue:688` and `:829` only. **The root cause is this spec's own sentence** — round 1's *A withdrawn Source withdraws its cache* named "removing a Source or unconfirming its typing" and there are five commands, not two.
3. **`sourceKey(node.id)` at `core/exec/execute.js:260` is uncontained** while the `stepKey` call beside it is. Not reachable in the shipped app — `ui/EditorPane.vue`'s `keyOfSource` goes through `stepZeroKey`, which contains internally — so it is a hole in `executeGraph`'s contract rather than a live defect, and the round-1 task said "both call sites" while meaning two of the three.
4. **`DEFAULT_MAX_ENTRIES = 1_000`'s rationale is arithmetically false.** The doc claims the ceiling "only ever binds on entries that cost no rows". 1,000 entries × 10,000 rows is 10,000,000, under `DEFAULT_MAX_ROWS` — so for any workload averaging under 15,000 rows per entry, which is the ordinary departmental CSV, the entry ceiling binds first and the row ceiling never runs. Correctness is unaffected (an eviction is a miss) but the stated reason for the number is not the behaviour of the number.
5. **`keyOrNull` is a blanket `catch`** and swallows `stepKey`'s and `sourceKey`'s deliberate programming-error guards ("a missing input key is a caller's bug rather than a state") along with the `canonical` refusals it was written for. Three failure classes, one silent outcome.
6. **This spec contradicted itself in four places after round 1**, all introduced by the amendment rather than by the code: the Verification bullets still carried "It disagrees materially and in the safe direction" and "Confirm the digest figure in the built artefact" under a Design Notes section that had just retracted both; the Design Notes claimed `npm run test:e2e` covers the bundled `digestBytes` while the Verification section says a cached and an uncached run are indistinguishable to every e2e assertion; task 8 named `cache.test.js` for a comment that lives in `execute.test.js`; and the `maxRows: 0` contract narrowing was not recorded.

*What was amended.* The four self-contradictions are fixed in place. `ARCHITECTURE-SPINE.md`'s **Cross-cutting — logging** row is amended (owner decision, below). The *A withdrawn Source* paragraph below now names the rule rather than two of its instances, and six new tasks follow.

*Known-bad state avoided.* Shipping an AD-29 fix that one deleted attribute disables silently, and a spec whose Verification section asserts what its Design Notes retract — which is worse than either error alone, because the next reader cannot tell which half is current.

*KEEP.* Everything from round 1's KEEP list, plus round 1's own work: `keyOrNull`'s containment, the symmetric `stepZeroKey` guard, the two ceilings and their `||`, the `clear()`-on-withdrawal primitive with its "coarse and correct" reasoning, and the four ledger entries. Round 2 sharpens these; it does not replace them.

*Owner decisions taken this round.* **`console.warn` in `core/` stays, and the architecture rule is amended rather than bypassed** (2026-08-05). The implementer had granted itself a scoped `eslint.config.js` allowance whose comment asserted the conventions row still held; it did not. `ARCHITECTURE-SPINE.md`'s logging row now states the exception and its four conditions, one of which — **emit once per distinct message** — reverses round 1's deliberate non-deduplication: `stepZeroKey` is reached from `ui/SourcesPane.vue`'s template and from `keyOfSource` on every run, so an un-deduplicated warning fires at keystroke frequency and buries the errors it sits among.

**2026-08-05 — review round 3. One defect class has now recurred three times, each round one level up, and two of round 2's instructions were in tension with each other.**

*The pattern, named because chasing it a fourth time would be the mistake.* "The wiring that makes the cache exist in the product is not covered by any test" has been the top finding in all three rounds. Round 0: deleting `:cache="runCache"` left the suite green. Round 2 fixed that and introduced the same hole for `:run-cache`. Round 3 finds it a level up again: round 2 moved the cache from a `setup` constant into `runCache: { type: Object, default: () => createRunCache() }`, and `ui/App.test.js`'s `render` helper takes `runCache = createRunCache()` as a **default parameter**, so all four cases inject a cache and none exercises App's own default. `app/main.js:42-48` passes no `runCache`, so that factory is the only thing giving the shipped artefact a cache at all. **Verified here by mutation, twice:** `default: null` and `default: () => createRunCache({ maxEntries: 1 })` each leave all 1027 tests green and `npm run lint` clean, with the product recomputing every Step on every view switch. The fix is not another binding assertion — it is that **at least one App case must mount the way `app/main.js` mounts**, with no cache prop at all, and observe caching through the engine.

*Two instructions of mine that contradicted each other.* Round 2 asked for both "wrap the `sourceKey` door in `keyOrNull`" and "narrow the `catch` so `stepKey`'s and `sourceKey`'s documented caller-bug guards are not swallowed". Those cannot both hold at one site with one mechanism, and narrowing won: `keyOrNull` now contains only `CanonicalRefusal`, so the door is wrapped in something that does not catch what comes through it. **Probed here:** an injected `sourceKey` throwing its own documented guard, and one throwing a plain `Error`, both escape `executeGraph` — the blank Editor the wrap was added to prevent. The resolution is that these are **two trust boundaries, not one**: an injected callback is foreign code on a render path and everything it throws is contained; an internal call to `stepKey` from a module that knows its own contract keeps propagating a caller bug. Two mechanisms because there are two boundaries.

*The rest.* `reparse`'s `withdraw()` sits in a `finally` that fires when **no store command was issued at all** — `setDelimiter('')`, `setSheet('')` and `setHeaderRow` with any non-integer or `< 1` value all hand it a `run` returning `undefined`, so a single keystroke in the Kopfzeile field discards the entire run cache. Verified. My earlier acceptance of the `finally` was reasoned about the *failed re-read* case only, where the placement is right; the no-op case is a different thing and is a performance defect on the exact path this story exists to make fast. Also: `no-console` is not actually enforced anywhere, so the register the amended architecture row points at is not a gate — `globalThis.console.warn` appended to `core/exec/cache.js` passes `npm run lint` clean, which makes both that row and the `eslint.config.js` comment assert an enforcement that does not exist. That row is mine and the claim has to become true or come out.

*KEEP.* Everything from rounds 1 and 2. Round 3 changes no mechanism: the key scheme, the two ceilings, `clear()`-on-withdrawal, `CanonicalRefusal`, the dedup and every test that fails on a mutation all stand.
...
**What this measurement does and does not establish**, recorded in review round 1 because the first wording overstated it: the loop was *injected* into the loaded page and timed there, not driven through `addSource`. So it pins the speed of the four-lane loop in each engine and the fact that all three engines agree on the 32 hex characters for the same input — but it does not exercise the bundled, minified `digestBytes` in `dist/index.html`. **Agreement between the bundled function and the Node one is therefore not verified by anything** — an earlier wording of this paragraph claimed `npm run test:e2e` covered it, which is false and contradicted the Verification section twenty lines below: e2e proves the bundled `digestBytes` does not throw, and nothing more, because a digest that is wrong-but-self-consistent changes no rendered count. Corrected in review round 2 and carried to the ledger rather than papered over. The browser cost is 1.7–1.9× Node's at every size and the two engines are within 8 % of each other, so the check's own threshold — "more than roughly double the Node figure is worth reporting before shipping" — is not crossed. At the design shape this is **43.6 / 47.0 ms once per ingest**, beside the 548–555 ms Step zero already costs on the same file; a run never pays it.
...
- `npx vitest run --project core` -- expected: green, including the six re-keyed Step-zero cases in `core/exec/convert.test.js`, the `cache-key` and `cache` suites, and round 1's additions: the entry-ceiling block in `cache.test.js`, the containment and null-table cases in `execute.test.js`, the two refusal paths in `convert.test.js`, and the real-ingest keying block in `source-store.test.js`
- `npx vitest run --project ui` -- expected: green, including the new `ui/App.test.js`; `graphLabelGaps()` and `kindLabelGaps()` still `[]` (this story mints no code, so nothing should have moved)
