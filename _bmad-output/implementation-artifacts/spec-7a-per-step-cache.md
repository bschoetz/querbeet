...
Against R4's 180 MB for the same configuration, so **the 15,000,000-row bound is conservative rather than optimistic and stays as it is.** This is Ask First territory and the constant was therefore not touched: it disagrees materially, and the direction happens to be the harmless one, but "the bound could be larger" is the owner's call and not the implementer's. Two caveats belong with the figure. It is measured over the Step kinds that *exist*: both share their input's columns and retain a mask or an index, which is `adapters/arquero/engine.js`'s decision and is stated there — a kind that materializes new column data (Aggregate and Join, stories 8 and 9) would retain real bytes and is not in this measurement. And the same table shows what the story is for from the other side: a repeat run over 30 Steps at half a million rows costs **0.8–1.4 ms against 484–3,097 ms**.
