...
- [ ] `core/exec/scheduler.test.js` -- a case with a counting `sourceTable` and a hand-driven yield asserting the count is **0** immediately after `startRun` returns -- the assertion that would have caught round 1, in the same shape as the existing `pending()` checks
...
- [ ] **Ten defects round 1 shipped, each one confirmed against the tree — none may recur:**
