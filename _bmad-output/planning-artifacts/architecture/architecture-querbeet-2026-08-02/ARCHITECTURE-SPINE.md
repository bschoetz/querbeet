...
- **Rule:** a date-typed column holds **UTC-midnight epoch milliseconds**; a datetime column holds UTC epoch milliseconds; a **time**-typed column holds **milliseconds since midnight**; a **duration**-typed column holds **plain milliseconds**. Readers normalize on the way in — a parser yielding local midnight and a Parquet reader yielding epoch days both converge here — and writers format from it on the way out. No `Date` object with an implicit local zone crosses a Step boundary. The display locale is a rendering concern and never reaches a Table or a Recipe.
- **Resolution is milliseconds, in all four.** Sub-millisecond digits are out of the representation and stay out: a Parquet TIMESTAMP in MICROS or NANOS loses its finer digits, and the loss is *named* per column (`parquet.timestamp_precision`, with the unit) rather than silent. That was the open question story 4 left for story 4a, and this is the answer — carrying microseconds would change this rule rather than any one adapter, and nothing in the MVP reads or reports below a millisecond. A detected ISO fraction of up to nine digits is counted readable and truncated here.
