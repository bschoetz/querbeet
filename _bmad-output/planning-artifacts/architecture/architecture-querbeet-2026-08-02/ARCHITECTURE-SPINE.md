...
### AD-21 — One temporal representation inside a Table `[ADOPTED, amended]`
...
- **Rule:** every temporal column holds **nanoseconds as a `BigInt`** — a date column UTC-midnight epoch nanoseconds, a datetime column UTC epoch nanoseconds, a **time** column nanoseconds since midnight, a **duration** column plain nanoseconds. Readers normalize on the way in — a parser yielding local midnight and a Parquet reader yielding epoch days both converge here — and writers format from it on the way out. No `Date` object with an implicit local zone crosses a Step boundary. The display locale is a rendering concern and never reaches a Table or a Recipe.
- **One unit for all four, and it is `BigInt`.** The unit is shared deliberately: a split rule — milliseconds for a date, something finer for a datetime — would make a date-against-datetime join a unit conversion, which is the class of error this decision exists to prevent. The unit is nanoseconds because a `datetime2(7)` timestamp out of SQL Server (`2026-02-13 15:57:35.4616727`) is an ordinary CSV export shape and its 100-nanosecond ticks are **already past `Number.MAX_SAFE_INTEGER`** — `1.77e16` against `9.007e15`, measured. Microseconds are the last resolution a `Number` holds exactly (until 2255) and cover six of those seven digits, so they would cost this same amendment and still round the value. Nine fractional digits is therefore both what detection reads and what the representation holds, rather than an arbitrary cap.
- **What `BigInt` costs, named rather than discovered.** `BigInt` and `Number` do not mix in arithmetic — a mean over a temporal column throws rather than coerces — so the `TableEngine` adapter absorbs it under AD-19's rule that the adapter carries the hazard and Step kinds never do. JSON has no `BigInt`, so the canonical serializer (AD-8, story 14) spells the encoding out rather than letting `JSON.stringify` throw at the first Recipe save.
- **A finer digit can still be lost before the representation sees it**, and that is a reader's limit rather than this rule's: hyparquet hands over a `Date`, which is millisecond-resolution by construction, so a Parquet TIMESTAMP in MICROS or NANOS arrives already rounded. The loss is *named* per column (`parquet.timestamp_precision`, with the unit) rather than silent, and closing it is a reader change, not an amendment here.
- **`time` and `duration` are choices a person makes, never declarations a file makes.** Both are `settable: true, native: false` in `core/types/catalog.js`: Parquet's `TIME` remains a refused declaration (story 4's closed list), so a duration column exists because the values pass 24:00 or because a user said so. `duration` exists as a type at all because the time-against-duration ambiguity is only reportable if both answers are choosable — an `unresolved` verdict whose alternatives include something the type select cannot offer would be a question with no answer.
...
- **Rule:** Chromium is the lead engine and every performance claim is measured there; Firefox is measured rather than assumed and is dropped rather than specially accommodated. There are **three envelopes and which code runs in which is the rule**: `core/`, `ports/` and `adapters/` under Vitest with **no browser** — `environment: 'node'` is an assertion, since a core test needing a DOM means AD-2 broke upstream of it; `ui/` under Vitest with **happy-dom**, for component behaviour that exists only inside a render function; and the **built artefact** under Playwright from a `file://` URL, subject to AD-18's Chromium-only network caveat and AD-16's untestable isolation caveat. The three are separate Vitest projects and a separate runner, never one config with a DOM switched on — a shared environment would hide exactly the violation the first envelope exists to catch. The `ui/` envelope is the narrowest of the three and is reached for last: state derivation belongs in `core/`, and pushing a `v-if` or an attribute binding there would mean rebuilding Vue inside a framework-free core. Under happy-dom `ResizeObserver` and `getBoundingClientRect` are unimplemented stubs while `scrollTop` is genuine (verified from source, R10); a `ui/` branch that needs real geometry goes to Playwright Component Testing, which the already-adopted `@playwright/test` carries. A rule stated here without a test that could fail is a rule that will rot.
...
| Cross-cutting — logging | No logging framework and no telemetry. Diagnostics are the only reporting channel **for anything a user can act on** — which is everything the product reports. One narrow exception, added 2026-08-05 by owner decision in story 7a review round 2: a **contained programming-error signal** may go to `console.warn`, under four conditions together — it names a state no user can cause or correct, it would otherwise be swallowed entirely, a Diagnostic would be the wrong shape (a German sentence on a card about machinery the product does not otherwise mention), and it is **emitted once per distinct message** rather than per occurrence, because a signal that repeats at render frequency buries the errors around it. The files that may name `console` are enumerated in `eslint.config.js` and nowhere else; a new one is an amendment to this row, not a lint edit. Today there is exactly one: `core/exec/cache-key.js`, where `keyOrNull` contains a `canonical` refusal that would otherwise escape onto a render path. This does not open `core/` to logging generally — AD-2's ban on `window`, `document`, `File`, `Blob`, `indexedDB` and `fetch` is untouched, and `console` qualifies on the same footing as `TextDecoder`/`TextEncoder`: present in Node and in every engine, so the core stays testable with no browser. |
