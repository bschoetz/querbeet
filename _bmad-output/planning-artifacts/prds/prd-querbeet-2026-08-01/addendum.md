...
**Re-verified 2026-08-02** during the architecture run, against the live registries rather than against this table. Twelve of thirteen pins are still current and none was fabricated or deprecated. Three corrections: `hyparquet-writer` **0.16.4** shipped on 2026-08-01, one patch ahead of the pin; **Arquero has had no upstream commit in fourteen months**, which is what the fork plan in §5 exists for and is now a fact rather than a precaution; and **the Vite major version was never pinned here at all** — only the plugin was — which matters because Vite 8 replaced its bundler and the plugin's single-file path under it has open upstream issues. That last one is an open question, resolvable only by building. Two claims about the built artifact were also *refuted* by measurement and are corrected in the spine rather than here: a `file://` network assertion cannot fail in Playwright's Firefox, and `read-excel-file` spawns its own blob-URL workers on the import path, so "querbeet creates workers only for the two exports" is true of querbeet's code and false of the bundle.
...
- **Every Step is a pure function of its inputs and its configuration.** ~~Over plain arrays of objects, since every engine examined consumes and produces that shape.~~ **Superseded 2026-08-02 by architecture decision AD-5.** What crosses a Step boundary is a `Table` handle behind querbeet's own narrow interface — `rows()`, `rowCount()`, `schema()`, `column()` — which the engine adapter implements; plain frozen row objects are materialized only at the real edges: preview, export, storage, worker transfer. The replaceability this seam exists for survives, but it moves from the row shape into the interface, so swapping the engine becomes one interface implementation rather than a per-operation change. The reason for the change is that the row shape would have been wrapped and unwrapped at every boundary, and at thirty Steps that cost could dominate the work — R4 measured only the whole pipeline, at 263–446 ms for 100,000 rows, and never measured a boundary. The spine is authoritative on this point; see `planning-artifacts/architecture/architecture-querbeet-2026-08-02/`.
