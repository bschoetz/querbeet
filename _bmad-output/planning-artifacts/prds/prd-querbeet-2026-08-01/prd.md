...
- **Detection reads every value in the column, not a sample.** Every comparable engine samples — DuckDB 20,480 rows, Arquero 1,000, Power Query 200, Frictionless 100 — and the value that resolves an ambiguous column, a day above 12 in `03/04/2025`, is decisive only inside the window that was scanned. That is the documented cause of silent corruption on large files. A querbeet column holds at most the NFR-3 design target and detection is already a walk over the column, so it reads all of it.
