...
3. **Does the graph survive machine authorship?** FR-28 requires a language model to emit a valid Recipe from documentation alone. A graph is materially harder to get right than a list. *Answered for the mechanism, not for the generality.* The format documents in 6,734 B; a full FR-27 block for four Sources and a running Pipeline is 10,938 B; thirteen defect classes are refused by name. **Gemini, given that block and nothing else in a session with no access to this repository, returned a valid Recipe on the first round** — 7 Steps, 6 edges, byte-stable across a round trip, identical to the reference answer Step for Step, and with the pre-existing Union preserved verbatim rather than rebuilt. But it is one vendor and one task, and the example's Column Annotations name both hard decisions outright, so it shows a foreign model can *express* a plan in this format, not that it can *find* one. *Remaining resolution: a second vendor, a Profile with the annotations stripped, and a task the Step kinds cannot answer — protocol and apparatus in `spikes/recipe-llm-authorship-2026-08-01/findings.md`.*
