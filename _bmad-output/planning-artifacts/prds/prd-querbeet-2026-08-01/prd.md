...
3. **Does the graph survive machine authorship?** FR-28 requires a language model to emit a valid Recipe from documentation alone. A graph is materially harder to get right than a list. *Half answered.* The format exists and documents in 6,734 B; a full FR-27 block for four Sources and a running Pipeline is 10,938 B; Recipes written from that block alone load and round-trip, and thirteen defect classes are refused by name. But the block, the format and those test Recipes were written by the same model in the same session, which measures self-consistency and not authorship. *Remaining resolution: paste `spikes/recipe-llm-authorship-2026-08-01/prompt-block-example.txt` into an assistant with no access to this repository and run its answer through the spike's self-test. Protocol in that spike's `findings.md`.*
