...
3. **Does the graph survive machine authorship?** FR-28 requires a language model to emit a valid Recipe from documentation alone. A graph is materially harder to get right than a list. *Answered: yes, at the sample size stated.* The format documents in 6,734 B; a full FR-27 block for four Sources and a running Pipeline is 10,938 B; thirteen defect classes are refused by name. **Gemini and Sonnet 5, each given a block and nothing else in a session with no access to this repository, returned valid Recipes in four runs out of four, none needing a second round.** The three asked to extend the same Pipeline produced the same graph as each other and as the reference answer — differing only in `duplicateAudit` — and each preserved the pre-existing Union verbatim rather than rebuilding it. The fourth, asked for a grouped sum, named the three missing operations and invented no Step kind. *Caveats, all recorded: Sonnet 5 shares the vendor of the session that wrote the format, so the count is one foreign vendor plus one same-family model; no run produced a refusal, so the paste-the-error-back loop FR-28 requires is still unexercised; and all four wrote `"value": "1000"` as a string for a numeric comparison, which makes the type of a comparison value a convention rather than a decision — see FR-14 and R1/R5.* Apparatus and protocol: `spikes/recipe-llm-authorship-2026-08-01/`.
