...
- **Which Vite major is the single-file plugin pinned against?** Vite 8 replaced its bundler and the plugin's single-file path under it has open upstream issues. Resolvable only by building — the one-file assertion is the test.
