...
- **Does Microsoft Excel agree with LibreOffice on German number and date formats?** Narrowed on 2026-08-02 from "do the format codes work at all". The strategy is now decided and verified in one independent German-locale renderer — see `measured-constraints.md` — so what remains is confirmation in the one product that matters and cannot be automated here. It is a one-open-one-look check against the probe workbook, not an investigation.
