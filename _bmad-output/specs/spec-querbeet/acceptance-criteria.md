...
- **What is recomputed is decided by what a Step is, not by which Step it is.** Renaming or moving a Step recomputes nothing. Setting a configuration back to a value it already had recomputes nothing. Correcting a Source's delimiter or encoding recomputes that Source and everything below it, even though its id never changed.
- **A result served without recomputing carries the diagnostics it was computed with.** A repeat run reports the same warnings, in the same order, against the same Steps, and never reads clean over a warning it did not re-emit — which is the whole reason a cache is acceptable here at all.
- **Withdrawing a confirmation withdraws what was computed from it.** Removing a Source, unconfirming its typing, changing a column's type or re-parsing it discards every retained result derived from that typing rather than leaving it reachable.
