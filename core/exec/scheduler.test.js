...
describe('a run that is left alone', () => {
  ...
  it('reports progress once per Step, naming the Step and the length of the walk', async () => {
    const { graph, filter, columns } = chain()
    const seen = []
    await start(graph, {
    ...
    expect(seen).toEqual([
      { done: 0, total: 3, stepId: 'src:umsatz' },
      { done: 1, total: 3, stepId: filter },
      { done: 2, total: 3, stepId: columns },
    ])
  })
