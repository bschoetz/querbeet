...
test('a long run says where it is, and stops between two Steps when it is told to', async ({
  ...
  expect(await cancelRun(page).evaluate((el) => el.tagName)).toBe('BUTTON')
  await cancelRun(page).focus()
  ...
  await expect(editorStatus(page)).toContainText(`Von ${STEPS + 1} Steps`)
