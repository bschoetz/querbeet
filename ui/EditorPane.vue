...
/**
 * Whether that Source's typing is confirmed — gate 1's question, answered
 * **without converting anything**.
 *
 * `sourceTable` above is the converting door: `createStepZeroCache.of` runs
 * `convertSource` on a miss, measured at 548–555 ms per contributing Source at
 * design scale. The gate loop runs above the walk, so a gate that asked for a
 * table paid all of that synchronously, before the run had yielded once — the
 * uncancellable half-second this story exists to remove, shipped and reverted on
 * 2026-08-05. The predicate reads the same field `convertSource` reads before it
 * converts (`core/exec/convert.js`), so the two doors cannot disagree: the gate
 * says exactly what the conversion would have said, and the conversion happens at
 * the Source's own node in the walk, between two yields.
 *
...
function startExecution() {
  ...
  try {
    handle = startRun({
      ...
      // The run's own identity, handed to the callback rather than read off
      // `handle` — which at the first report may not have been assigned yet. The
      // scheduler's contract says it will have been; a signature that does not
      // need the contract is better than a comment that cites it.
      onProgress: (at, run) => {
  ...
  handle.completed.then(
    ...
    (thrown) => {
      ...
      // `watch`, where the framework's error handler saw it. It no longer does,
      // and the first version of this comment claimed otherwise: rethrowing inside
      // a rejection callback rejects *this* derived promise, which nobody holds,
      // so what the platform reports is an unhandled rejection — `console` in both
      // engines, and `window.onunhandledrejection` for anything that wants it.
      // That is the honest description and it is also the only channel available
      // here: `startExecution` is fire-and-forget by design (nine call sites,
      // none of them awaiting), so there is no caller to hand the error to, and
      // `ui/` is not in the register of files that may name `console` (the
      // architecture's Cross-cutting — logging row). Swallowing it would be worse
      // than either: `core/exec/execute.js` already catches everything a *Step*
      // can do, so anything arriving here is a programming error in the machinery
      // around them.
      //
      // What this handler owes the user is the band: cleared, so the pane does not
